...
// 二次.
//
// 本文件用到若干 C++ 2.0 New Features，所以你必須在你的集成環境 (IDE) 上設定
// "C++11 on". 如果你的編譯器報錯，應是因為我用到 (測試) GNU 標準庫的獨特
// (非標準) 組件 (特別是 allocators)， 將報錯語句改為註解 (comments)
// 即可通過編譯.
...
void test_misc() {
    ...
    cout << "MyStrNoMove(zoo) == MyStrNoMove(hello) ==> "
         << (MyStrNoMove("zoo") == MyStrNoMove("hello")) << endl;  // 0
    // 以上建構了 6 個 MyString objects 和 4 個 MyStrNoMove
    // objects，都是暫時生命.
}
}  // namespace jj00
//--------------------------------------------------
...
void test_unordered_multiset(long& value) {
    ...
    cout << "unordered_multiset.max_size()= " << c.max_size()
         << endl;  // 357913941
...
void test_unordered_multimap(long& value) {
    ...
    cout << "unordered_multimap.max_size()= " << c.max_size()
         << endl;  // 357913941
...
void test_stack(long& value) {
...

    {
        ...
        //[Error] 'class std::set<std::basic_string<char> >' has no member named
        'push_back'
        //[Error] 'class std::set<std::basic_string<char> >' has no member named
        'back'
        //[Error] 'class std::set<std::basic_string<char> >' has no member named
    ...
    //! stack<string, map(string>> c5;	////以 map 為底層, [Error] template
    //! argument 2 is invalid stack<string>::iterator ite1;  	//[Error]
    //! 'iterator' is not a member of 'std::stack<std::basic_string<char> >'
}
}  // namespace jj17
//---------------------------------------------------
#include <cstdio>   //snprintf()
...
void test_queue(long& value) {
    ...
    //! argument 2 is invalid queue<string>::iterator ite1;  	//[Error]
    //! 'iterator' is not a member of 'std::queue<std::basic_string<char> >'
}
}  // namespace jj18
//---------------------------------------------------
#include <algorithm>  //find()
...
void test_list_with_special_allocator() {
    ...
    p = alloc4.allocate(2);
    alloc4.deallocate(p,
                      2);  // 我刻意令參數為 2, 但這有何意義!! 一次要 2 個 ints?
    ...
    p = alloc6.allocate(3);
    alloc6.deallocate(p,
                      3);  // 我刻意令參數為 3, 但這有何意義!! 一次要 3 個 ints?
#endif
}
}  // namespace jj20
//---------------------------------------------------
...
void test_components_sizeof() {
    ...
    //! << endl;	//[Error] 'iterator' is not a member of
    //! 'std::stack<int>' cout << "sizeof(queue<int>::iterator)= " <<
    //! sizeof(queue<int>::iterator) << endl;	//[Error] 'iterator' is not a
    //! member of 'std::queue<int>
